...
```js
function formatDataTree1(source) {
  return source.filter(item => {
    const children = source.filter(c => c.pid === item.id);
...
```js
function formatDataTree2(source) {
  const parent = source.filter(item => item.pid === 0);
  function concatChildren(source, parent) {
    parent.forEach(p => {
      source.forEach(c => {
        if (p.id === c.pid) {
          p.children ? p.children.push(c) : (p.children = [c]);
          concatChildren(source, [c]);
        }
      });
    });
  }
  concatChildren(source, parent);
  return parent;
}
```
...
```js
function formatDataTree3(source, pid) {
  const result = [];
  function getChildren(source, result, pid) {
    source.forEach(item => {
      if (item.pid === pid) {
